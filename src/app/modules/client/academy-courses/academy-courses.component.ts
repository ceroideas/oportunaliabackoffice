...
export class AcademyCoursesComponent implements OnInit {
	...
	initTable(): void {
		...
		this.dtOptions = {
			...
			ajax: {
				url: endpoint('academy_courses_dt'),
				type: 'get',
				headers: {
			...
			columns: [
				{
					title: 'Título', 
					data: function(row){return '<span style="display:none">'+that.normalizeText(row.title)+'</span>'+row.title},
				},
				{
					title: 'Precio', 
					data: function(row) {
						return row.is_free ? '<span class="badge badge-success">Gratis</span>' : row.price + ' €';
					}
				},
				{
					title: 'Estado', 
				...
				},
				{
					title: 'Estudiantes', 
					data: function(row) {
						return row.stats?.total_students || 0;
					}
				},
				{
					title: 'Visualizaciones', 
					data: function(row) {
						return row.stats?.total_views || 0;
					}
				},
				{
					title: 'Orden', 
			...
			rowCallback: (row: Node, data: any[] | Object, index: number) => {
				...
				$('button.ver', row).bind('click', () => {
					that.router.navigate(['/academy/courses', data['id']]);
				});

				$('button.editar', row).unbind('click');
				$('button.editar', row).bind('click', () => {
					that.router.navigate(['/academy/courses', data['id'], 'edit']);
				});
	...
	deleteCourse(): void {
		...
		this.dataService.http.delete(endpoint('academy_courses_delete', { id: this.deleteId }), { headers: this.dataService.headers })
			.toPromise()
			.then((response: any) => {
				this.utils.logResponse(response);
				if (response.code === 200) {
					this.utils.showToast('Curso eliminado correctamente', 'success');
					this.modalRef.hide();
					this.dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
						dtInstance.ajax.reload();
					});
				} else {
					this.utils.showToast('Error al eliminar curso', 'error');
				}
			})
			.catch((error: any) => {
				console.error(error);
