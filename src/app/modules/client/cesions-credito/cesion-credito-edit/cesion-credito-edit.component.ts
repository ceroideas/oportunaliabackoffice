...
@Component({
  selector: 'app-cesion-credito-edit',
  templateUrl: './cesion-credito-edit.component.html',
  styleUrls: ['./cesion-credito-edit.component.scss']
})
export class CesionCreditoEditComponent implements OnInit {
  ...
  ngOnInit(): void {
	...
	saveCesion(status = null) {
		...
		if (this.form.valid) {
			...
			this.auctionsService.updateCesionCredito(this.cesionId, data)
			.subscribe(data => {
				this.router.navigate(['/cesions-credito']);
