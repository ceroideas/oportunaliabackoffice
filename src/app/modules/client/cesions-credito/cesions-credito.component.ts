...
@Component({
  selector: 'app-cesions-credito',
  templateUrl: './cesions-credito.component.html',
  styleUrls: ['./cesions-credito.component.scss']
})
export class CesionsCreditoComponent implements OnInit {
  ...
  onFileSelected(event: Event): void {
    ...
    onSubmit(): void {
        if (this.file) {
            ...
            formData.append('type', "cesion_credito");
  ...
  ngOnDestroy(): void {
    ...
    let that = this;
		...
		this.dtOptions = {
			...
			ajax: {
				url: endpoint('cesions_credito_dt'),
				type: 'get',
				headers: {
				...
				},
        {
			...
			rowCallback: (row: Node, data: any[] | Object, index: number) => {
				...
				$('button.editar', row).bind('click', () => {
					that.router.navigate(['/cesions-credito', data['id'], 'edit']);
				});
	...
	reloadTable() {
		...
		let newUrl = endpoint('cesions_credito_dt', params);
	...
	featureCesion(id) {
		...
		this.auctionsService.featureCesionCredito(id)
		.subscribe(data => {
			this.reloadTable();
	...
	asignarCesion(id) {
		...
		this.auctionsService.asignarCesionCredito(id)
		.subscribe(data => {
			this.reloadTable();
	...
	duplicateCesion(id: number){
		...
		this.auctionsService.duplicateCesionCredito(id)
		.subscribe(data => {
			this.reloadTable();
			this.utils.showToast('Cesión de crédito duplicada');
		});
	...
	deleteCesion() {
		...
		this.auctionsService.deleteCesionCredito(this.deleteId)
		.subscribe(data => {
			this.reloadTable();
			this.utils.showToast('La cesion de crédito ha sido borrada');
			this.modalRef.hide();
		});
	}
	...
	finalReport(id: number) {
		...
		this.auctionsService.cesionCreditoFinalReport(id)
		.subscribe(data => {
			this.utils.downloadFile(data, 'pdf', 'Informe final de cesion de crédito '+id);
		});
	}
	...
	importCesions() {
		this.modalRef.hide();
		this.reloadTable();
		this.utils.showToast('Cesiones de crédito importadas correctamente');
	}
}