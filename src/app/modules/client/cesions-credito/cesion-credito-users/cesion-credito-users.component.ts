...
@Component({
  selector: 'app-cesion-credito-users',
  templateUrl: './cesion-credito-users.component.html',
  styleUrls: ['./cesion-credito-users.component.scss']
})
export class CesionCreditoUsersComponent implements OnInit {
  ...
  constructor(
	...
	ngOnInit(): void {
		...
		this.route.params.subscribe(params => {
			...
			if (!params['id']) { this.router.navigate(['/cesions-credito']); }
  ...
  ngOnDestroy(): void {
	...
	getAuction() {
		...
		this.auctionsService.getCesionCredito(this.cesionId)
		.subscribe((data: BaseResponse<Auction>) => {
			...
			this.router.navigate(['/cesions-credito']);
		});
	}
	...
	initTable(): void {
		...
		this.dtOptions = {
			...
			ajax: {
				url: endpoint('cesion_credito_users_dt', { id: this.cesionId }),
				type: 'get',
				headers: {
			...
			initComplete: function(settings, json) {
				...
				let table_id = '#cesion-credito-usuarios';
  ...
  reloadTable() {
		...
		let newUrl = endpoint('cesion_credito_users_dt', { id: this.cesionId });
  ...
  confirmValidateOffer(id) {
	...
	validateOffer(status: number) {
		...
		this.auctionsService.validateOfferCredito(this.validateId, status)
		.subscribe(data => {
			if (status == 1) {
