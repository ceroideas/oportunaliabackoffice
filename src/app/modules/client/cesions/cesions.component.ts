...
@Component({
  selector: 'app-cesions',
  templateUrl: './cesions.component.html',
  styleUrls: ['./cesions.component.scss']
})
export class CesionsComponent implements OnInit {
  ...
  ngOnDestroy(): void {
				...
				},
        {
			...
			rowCallback: (row: Node, data: any[] | Object, index: number) => {
				...
				$('button.editar', row).bind('click', () => {
					that.router.navigate(['/cesions', data['id'], 'edit']);
				});
	...
	reloadTable() {
		...
		let newUrl = endpoint('cesions_dt', params);
	...
	featureCesion(id) {
		...
		this.auctionsService.featureCesion(id)
		.subscribe(data => {
			this.reloadTable();
	...
	asignarCesion(id) {
		...
		this.auctionsService.asignarCesion(id)
		.subscribe(data => {
			this.reloadTable();
	...
	duplicateCesion(id: number){
		...
		this.auctionsService.duplicateCesion(id)
		.subscribe(data => {
			this.reloadTable();
			this.utils.showToast('Cesión de remate duplicada');
		});
	...
	deleteCesion() {
		...
		this.auctionsService.deleteCesion(this.deleteId)
		.subscribe(data => {
			this.reloadTable();
			this.utils.showToast('La cesion de remate ha sido borrada');
			this.modalRef.hide();
		});
	}
	...
	finalReport(id: number) {
		...
		this.auctionsService.cesionFinalReport(id)
		.subscribe(data => {
			this.utils.downloadFile(data, 'pdf', 'Informe final de cesion de remate '+id);
		});
	}
