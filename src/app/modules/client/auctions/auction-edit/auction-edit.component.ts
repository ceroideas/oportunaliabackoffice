...
export class AuctionEditComponent implements OnInit {
	...
	getAuction() {
		...
		.subscribe((data: BaseResponse<Auction>) => {
			...
			if (data.code == 200) {
				...
				let startDate = this.utils.extractDateFrom(this.auction.start_date);
				let startTime = this.utils.extractTimeFrom(this.auction.start_date);
				this.auction.start_date = startDate;
				this.auction.start_time = startTime;

				let endDate = this.utils.extractDateFrom(this.auction.end_date);
				let endTime = this.utils.extractTimeFrom(this.auction.end_date);
				this.auction.end_date = endDate;
				this.auction.end_time = endTime;
			}

			this.form.patchValue(this.auction);
	...
	setFormFields() {
		...
		this.form = this.fb.group({
			...
			deposit: [''],
			minimum_bid: [''],
			bid_time_interval: ['', [Validators.required]],
			bid_price_interval: ['', [Validators.required]],
	...
	saveAuction(status = null) {

		this.submitted = true;

		if (this.form.valid) {

			this.utils.formToObject(this.form, this.auction);
			...
			data.append('id', this.auction.id.toString());
			...
			data.append('bid_time_interval', this.form.get('bid_time_interval').value);
			data.append('bid_price_interval', this.form.get('bid_price_interval').value);
	...
	gotoPublic() {
		window.open(this.auction.public_path, '_blank').focus();
	}
	...
	deleteDocument(document: any) {
		...
		this.auctionsService.deleteDocument(this.auctionId,document)
		.subscribe(data => {
