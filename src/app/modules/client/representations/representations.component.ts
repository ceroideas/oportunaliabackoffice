...
import { RepresentationType } from 'src/app/shared/models/user.model';
...
export class RepresentationsComponent implements OnInit {
	...
	initTable(): void {
		...
		this.dtOptions = {
			...
			ajax: {
				url: endpoint('representations_dt'),
				type: 'get',
				headers: {
			...
			columns: [
				...
				{
					title: 'Poderes', orderable: false, data: null, className: 'all text-center not-filterable',
					render: function (data, type, row, meta) {

						if (row.document?.path) {

							return `<button class="poder btn btn-table btn-navy m-0" title="${row.document.name}">
				...
				{
					title: 'Validar poderes', orderable: false, data: null, className: 'all text-center not-filterable',
					render: function (data, type, row, meta) {

						switch (row.status) {
							case 1: return `<button class="validar btn pill pill-green" title="Cambiar validación">Validados</button>`; break;
							case 2: return `<button class="validar btn pill pill-red" title="Cambiar validación">No validados</button>`; break;
			...
			initComplete: function(settings, json) {
				...
				let table_id = '#representaciones';

				$('thead tr th:nth-child(4)', table_id).css({ 'max-width': '100px' });
	...
	reloadTable() {
		...
		let newUrl = endpoint('representations_dt', params);
	...
	download(type) {
		...
		this.userService.representationsExport(type)
		.subscribe(data => {
			this.utils.downloadFile(data, type, 'Representaciones');
		});
	}
}
