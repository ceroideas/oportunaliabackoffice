...
@Component({
	selector: 'app-direct-selling-history',
	templateUrl: './direct-selling-history.component.html',
	styleUrls: ['./direct-selling-history.component.scss']
})
export class DirectSellingHistoryComponent implements OnInit, OnChanges {

	public lastBidLabel: string = '';
	public timeLeftLabel: string = '';

	@Input() directSellingId!: number;
	...
	initTable(): void {
		...
		this.dtOptions = {
			...
			dom: `tip`,
			ajax: {
				url: endpoint('direct_selling_history_dt', { id: this.directSellingId }),
				type: 'get',
				headers: {
				...
				dataSrc: function (json) {
					...
					that.directSelling = json['response']['auction'];

					that.images = that.directSelling.images;
			...
			initComplete: function(settings, json) {
				...
				let table_id = '#venta-directa-ofertas';
	...
	reloadTable() {
		...
		let newUrl = endpoint('direct_selling_history_dt', { id: this.directSellingId });
	...
	countdown() {
		...
		if (this.directSelling) {
			...
			if (this.directSelling.auction_status_id == AuctionStatus.ONGOING) {

				this.time_left = this.utils.timeDiffString(`${this.directSelling.end_date}`);
			}
