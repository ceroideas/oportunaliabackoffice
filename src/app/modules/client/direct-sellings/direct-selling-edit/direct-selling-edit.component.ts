...
@Component({
	selector: 'app-direct-selling-edit',
	templateUrl: './direct-selling-edit.component.html',
	styleUrls: ['./direct-selling-edit.component.scss']
})
export class DirectSellingEditComponent implements OnInit {
	...
	getAuction() {
		...
		.subscribe((data: BaseResponse<Auction>) => {
			...
			if (data.code == 200) {
				...
				let styles = this.auctionsService.auctionStatus(this.directSelling);
				this.directSellingStatus = styles.text;
				this.directSellingStatusColor = 'text-'+styles.color;

				let startDate = this.utils.extractDateFrom(this.directSelling.start_date);
				let startTime = this.utils.extractTimeFrom(this.directSelling.start_date);
				this.directSelling.start_date = startDate;
				this.directSelling.start_time = startTime;

				let endDate = this.utils.extractDateFrom(this.directSelling.end_date);
				let endTime = this.utils.extractTimeFrom(this.directSelling.end_date);
				this.directSelling.end_date = endDate;
				this.directSelling.end_time = endTime;
			}

			this.form.patchValue(this.directSelling);
	...
	saveDirectSelling(status = null) {
		...
		if (this.form.valid) {
			...
			data.append('id', this.directSelling.id.toString());
			...
			this.auctionsService.editDirectSelling(data, this.directSellingId)
			.subscribe(data => {
				this.router.navigate(['/direct-sellings']);
	...
	gotoPublic() {
		window.open(this.directSelling.public_path, '_blank').focus();
	}
	...
	deleteDocument(document: any) {
		...
		this.auctionsService.deleteDocument(this.directSellingId,document)
		.subscribe(data => {
