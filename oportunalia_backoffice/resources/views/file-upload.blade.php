...
<html>
    ...
    <body>
        ...
        <div class="container mt-4">
            ...
            <h2 class="text-center">Importador masivo de activos</h2>
            @isset($message)
                <div class="alert alert-success mt-1 mb-1">{{ $message }} puedes descargar el informe en:
                    <a href={{url('documents/uploads/importactives').date('d-m-y').'.log'}} target="_blank"> {{url('documents/uploads/importactives').date('d-m-y').'.log'}}</a>
                </div>
            ...
            @endisset
            <br>
            <h4>1. Subir imagenes y documentos de activos</h4>
            <form action="{{ url('api/upload-files') }}" class="dropzone" id="my-dropzone" method="post" enctype="multipart/form-data">
                @csrf
            </form>
            <br>
            <h4>2. Importar fichero de activos y ventas</h4>
            <form method="POST" enctype="multipart/form-data" id="upload-file" action="{{ url('api/store-actives') }}" >
                @csrf
                <div class="row">
                    <div class="col-md-12">
