...
class CreditAssignmentController extends ApiController
{
    public function __construct()
    {
        ...
        •              Inmueble procedente de ejecución hipotecaria en fase de cesión de crédito
    ...
    public function listAll(Request $request)
    {
        ...
        ->where("auction_type_id", 4) // Cambiado a 4 para cesiones de crédito
        ->groupBy("auctions.id");
    ...
    public function create(Request $request)
    {
        ...
        else
        {
            ...
            $technical_archive_id = null;
            $description_archive_id = null;
            $land_registry_archive_id = null;
            $conditions_archive = null;
            $technical_archive_id_two = null;
            ...
            if($request->file("description_document_two") != null && $request->file("description_document_two") != ""){
                $fileDescriptionTwo = Storage::disk("public")->put("", $request->file("description_document_two"));
                $description_archive_two = Archive::create(["name" => $request->file("description_document_two")->getClientOriginalName(), "path" => $fileDescriptionTwo]);
                $description_archive_two_id = $description_archive_two->id;
            }

            if($request->file("technical_document_two") != null && $request->file("technical_document_two") != ""){
                $fileTechnicalTwo = Storage::disk("public")->put("", $request->file("technical_document_two"));
                $technical_archive_two = Archive::create(["name" => $request->file("technical_document_two")->getClientOriginalName(), "path" => $fileTechnicalTwo]);
            ...
            if($request->file("land_registry_document_two") != null && $request->file("land_registry_document_two") != ""){
                $fileLandRegistryTwo = Storage::disk("public")->put("", $request->file("land_registry_document_two"));
                $land_registry_archive_two = Archive::create(["name" => $request->file("land_registry_document_two")->getClientOriginalName(), "path" => $fileLandRegistryTwo]);
            ...
            if($request->file("conditions_document_two") != null && $request->file("conditions_document_two") != ""){
                $fileConditionsTwo = Storage::disk("public")->put("", $request->file("conditions_document_two"));
                $conditions_archive_two = Archive::create(["name" => $request->file("conditions_document_two")->getClientOriginalName(), "path" => $fileConditionsTwo]);
            ...
            $auction = Auction::create(array_merge($validator->validated(),
                [
                    ...
                    'auction_type_id' => 4, // Cesiones de crédito
    ...
    public function detail(Request $request, $id)
    {
        ...
        $auction->public_path = url('/cesion-credito/'.$auction->link_rewrite);
    ...
    public function edit(Request $request, $id)
    {
        ...
        if ($request->file("land_registry_document")) {
            ...
            $archiveDeleteLandRegistry = Archive::find($auction->land_registry_archive_id);
        ...
        if ($request->file("description_document_two")) {
            $fileDescriptionTwo = Storage::disk("public")->put("", $request->file("description_document_two"));
            $archive = Archive::create(["name" => $request->file("description_document_two")->getClientOriginalName(), "path" => $fileDescriptionTwo]);
            $archiveDeleteDescriptionTwo = Archive::find($auction->description_archive_two_id);
            if($archiveDeleteDescriptionTwo){
                Storage::disk("public")->delete($archiveDeleteDescriptionTwo->path);
            }
        ...
        if ($request->file("technical_document_two")) {
            $fileTechnicalTwo = Storage::disk("public")->put("", $request->file("technical_document_two"));
            $archive = Archive::create(["name" => $request->file("technical_document_two")->getClientOriginalName(), "path" => $fileTechnicalTwo]);
            $archiveDeleteTechnicalTwo = Archive::find($auction->technical_archive_two_id);
            if($archiveDeleteTechnicalTwo){
                Storage::disk("public")->delete($archiveDeleteTechnicalTwo->path);
            }
        ...
        if ($request->file("land_registry_document_two")) {
            $fileLandRegistryTwo = Storage::disk("public")->put("", $request->file("land_registry_document_two"));
            $archive = Archive::create(["name" => $request->file("land_registry_document_two")->getClientOriginalName(), "path" => $fileLandRegistryTwo]);
            $archiveDeleteLandRegistryTwo = Archive::find($auction->land_registry_archive_two_id);
            if($archiveDeleteLandRegistryTwo){
                Storage::disk("public")->delete($archiveDeleteLandRegistryTwo->path);
            }
        ...
        if ($request->file("conditions_document_two")) {
            $fileConditionsTwo = Storage::disk("public")->put("", $request->file("conditions_document_two"));
            $archive = Archive::create(["name" => $request->file("conditions_document_two")->getClientOriginalName(), "path" => $fileConditionsTwo]);
            $archiveDeleteConditionsTwo = Archive::find($auction->conditions_archive_two_id);
            if($archiveDeleteConditionsTwo){
                Storage::disk("public")->delete($archiveDeleteConditionsTwo->path);
            }
    ...
    public function duplicate(Request $request, $id)
    {
        ...
        if($auction->auction_status_id == 1 ||$auction->auction_status_id == 5 ||$auction->auction_status_id == 7){
            ...
            $new_active_name = $new_active->name . " - copia Cesión de crédito";
        ...
        $new_auction->title = $new_auction->title . " - copia Cesión de crédito";
    ...
    public function export($type)
    {
        $filename = [
            config('app.name'),
            __('fields.backoffice.credit_assignments'),
        ];
        ...
        switch ($type)
        {
            case 'xlsx':
                ...
                return \Maatwebsite\Excel\Facades\Excel::download(
                    new \App\Exports\Admin\CreditAssignmentExport, $filename
                );

            case 'offerscreditassignments':
                $filename = composeFilename($filename, 'xlsx');

                return \Maatwebsite\Excel\Facades\Excel::download(
                    new \App\Exports\Admin\CreditAssignmentOfferExport, $filename
                    );
    ...
    public function syncWithFotocasa($auction) {
        try {
            ...
            $fotocasaData = [
                ...
                "PropertyDocument" => array_map(function ($image) {
                    static $index = 0;
                    return [
                        'TypeId' => 1, 
                        'Url' => $image['path'], 
