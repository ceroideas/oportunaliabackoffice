...
class CreditAssignmentOfferExport implements FromCollection, WithColumnFormatting, WithColumnWidths, WithHeadings, WithMapping, WithStyles
{
	...
	public function collection()
	{
        ...
        $creditAssignmentOffersQuery = DirectSaleOffer::with([
            'user',
        ])
         ->join("auctions", "direct_sale_offers.auction_id", "=", "auctions.id")
         ->where("auctions.auction_type_id", 4)
         ->where("direct_sale_offers.created_at", ">",$lessDate);

        return $creditAssignmentOffersQuery->get();
	}
	...
	{
        $tipo_venta = $item->auction_type_id == 2 ? 'Venta directa':'Cesion de crédito';
